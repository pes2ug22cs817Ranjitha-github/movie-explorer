  ...
  return (
    <div className="p-6 bg-white dark:bg-gray-900 dark:text-white min-h-screen">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
        {movies.map((movie) => (
          <div key={movie.id} className="relative group">
            <Link href={`/movie/${movie.id}`}>
              <div className="bg-white dark:bg-gray-800 dark:text-white rounded-lg shadow p-2 hover:shadow-md transition-shadow">
                <Image
                  src={
                    movie.poster_path
        ...
        {loadingInitial && Array.from({ length: 8 }).map((_, i) => <MovieSkeleton key={i} />)}
      </div>
      <div ref={observerRef} className="h-10 mt-10 text-center text-gray-500 dark:text-gray-400">
